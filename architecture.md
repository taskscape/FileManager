...
- A transaction-wide first-error latch covers the host registry helpers and plug-in calls through `CSalamanderRegistry`, including operations dispatched to the registry worker. Required create, set, clear, delete, flush, and close failures prevent payload acceptance. Missing values during optional deletion remain benign. A rejected save retains pending settings and emits one failure message after releasing the worker and mutex; a subsequent explicit save or settings commit retries them.
- New generations carry a checksummed `Payload Integrity Version`, intended item counts and required field validation for six numbered host collections: custom packers/unpackers, predefined packers, archive associations, user-menu items, and highlighting entries. Counts describe intended in-memory contents, so a missing child cannot be legitimized by checksumming the partial tree. Older verified generations without this manifest remain readable.
...
- Startup captures the `Transaction Identity` GUID from the generation actually loaded. Optional fallback retirement reacquires the save mutex, validates the current selected generation, and deletes the opposite slot only if its GUID matches that startup identity. Slot reuse, intervening saves, missing legacy identities, or uncertain lock/validation results retain the fallback.
