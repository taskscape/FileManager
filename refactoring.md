...
- **Delivered:** Main-push release runs now execute the complete `scripts/runtests.ps1 -FailOnSkipped` inventory on the dedicated isolated UI runner before packaging. A Release build can start only after that gate, and the `production`-protected publish job can release only the immutable installer artifact uploaded by the successful build job. Build and test jobs are read-only; only publish receives `contents: write`.
...
- **Native target (2026-08-14):** `tests/NativeSafetyTests/NativeSafetyTests.vcxproj` is a standalone C++ executable run by `scripts/runtests.ps1`. It executes checked-arithmetic overflow/narrowing boundaries, fault-adapter calls, and a self-owned Win32 copy, rename, move, and delete lifecycle directly in the native ABI. The product-level FlaUI suite remains responsible for user-facing conflict, cancellation, recovery, cross-volume, and recycle-bin behavior.
...
- **Verification:** `NativeSafetyRegressionTests.External_size_fields_use_checked_arithmetic_before_allocation_or_io` protects the helpers and every current external uploader, parser-broker, PictView thumbnail and WIC-surface, and ZIP local-header/SFX size field from reverting to unchecked arithmetic, and pins the removal of the PictView envelope sources.
...
- **Executable corpus/snapshot gate (2026-08-14):** `tools/test-7zip-compatibility.ps1` copies the exact built `7zwrapper.dll`/`7za.dll` pair to an isolated directory, archives the retained three-file corpus through the wrapper, and uses an independent `7z.exe`-compatible oracle to validate, extract, and compare both wrapper and oracle archives with the expected SHA-256 manifest. Header, payload, and footer bit-flip mutations are retained as deterministic fuzz regressions and must all be rejected. `scripts/runtests.ps1` records the gate and strict release runs fail if the oracle is absent.
...
- **Delivered:** `DeterministicNetworkFixtureTests` owns one-connection loopback HTTP, FTP, and FTPS servers. The scripts fragment protocol replies, close the FTP control connection after a terminal reply, negotiate a self-signed TLS 1.2 connection, and stall until the client deadline expires. In a disposable profile, `ProductFtpControlConnectionTests` discovers the FTP plug-in's runtime Quick Connect command, drives `127.0.0.1:port` through the native dialog, and verifies that the plug-in consumes the fragmented multiline greeting before reaching its login command and scripted disconnect boundary. `scripts/runtests.ps1` executes the profile-safe fixture group even when the destructive UI suite cannot run.
