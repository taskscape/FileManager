...
static BOOL ConvertFindDataWToUtf8Local(const WIN32_FIND_DATAW* src, WIN32_FIND_DATAA* dst)
{
    ...
    WideCharToMultiByte(CP_UTF8, 0, src->cFileName, -1, dst->cFileName, (int)sizeof(dst->cFileName), NULL, NULL);
    WideCharToMultiByte(CP_UTF8, 0, src->cAlternateFileName, -1, dst->cAlternateFileName, (int)sizeof(dst->cAlternateFileName), NULL, NULL);
    return TRUE;
}
...
__declspec(allocate(".i_hnd$a")) const _PVFV i_handles = (_PVFV)1; // place the i_handles variable at the beginning of the .i_hnd section

#pragma section(".i_hnd$z", read)
__declspec(allocate(".i_hnd$z")) const _PVFV i_handles_end = (_PVFV)1; // place the i_handles_end variable at the end of the .i_hnd section
...
*/
        {
            ConnectToTraceServer(); // in case the server wasn't launched
...
C__Handles::SetInfo(const char* file, int line, C__HandlesOutputType outputType)
{
    ...
    {
        DebugBreak(); // recursive call to handles !!! another masked message-loop - see call-stack
    }
#endif // MULTITHREADED_HANDLES_ENABLE
                              ...
                              C__HandlesOrigin* origin, C__HandlesType expType)
{
        ...
        if (Handles[i].Handle.Handle == handle)
        {
                ...
                expType == __htHandle_comp_with_DeleteObject && __Handles_IsCompWithDeleteObject(type))
            {
                ...
                if (org != __hoLoadAccelerators && org != __hoLoadIcon &&
                    org != __hoGetStockObject) // not a handle that doesn't need to be released (we prioritize releasing handles that must be released)
                {
                    if (origin != NULL)
                        *origin = org;
                    Handles.Delete(i);
    ...
    if (foundTypeOK != -1) // found only handle that may not be freed
    {
                        ...
                        HANDLE hTemplateFile)
{
    ...
    if (ret == INVALID_HANDLE_VALUE) // parameters to buffer only when error occurs (can be displayed)
    {
        ...
        _snwprintf_s(paramsBuf, _TRUNCATE,
                     L"dwDesiredAccess=0x%X,\ndwShareMode=0x%X,\ndwCreationDisposition=0x%X,\ndwFlagsAndAttributes=0x%X,\nlpFileName=%s", // lpFileName is intentionally at the end, it can be up to 32k long and will be truncated
...
HANDLE
C__Handles::CreateFileUtf8(LPCSTR lpFileName, DWORD dwDesiredAccess,
                           DWORD dwShareMode,
                           LPSECURITY_ATTRIBUTES lpSecurityAttributes,
                           ...
                           HANDLE hTemplateFile)
{
    ...
    const char* paramsA = NULL;
    const WCHAR* paramsW = NULL;
    char paramsABuf[MAX_PATH + 200];
    WCHAR paramsWBuf[800 + 200]; // limit file name length for messages
    if (ret == INVALID_HANDLE_VALUE)
    {
        ...
        {
            _snprintf_s(paramsABuf, _TRUNCATE,
                        "%s,\ndwDesiredAccess=0x%X,\ndwShareMode=0x%X,\ndwCreationDisposition=0x%X,\ndwFlagsAndAttributes=0x%X",
                        lpFileName != NULL ? lpFileName : "<null>",
                                 ...
                                 DWORD dwOptions)
{
    ...
    if (ret)
    {
        ...
        // GetCurrentProcess returns some kind of pseudohandle, so this construction
...
BOOL C__Handles::FreeLibrary(HMODULE hLibModule)
{
    ...
    BOOL ret = ::FreeLibrary(hLibModule); // contains calls to DLL global destructors, may contain message-loop
...
VOID C__Handles::FreeLibraryAndExitThread(HMODULE hLibModule, DWORD dwExitCode)
{
    ...
    ::FreeLibraryAndExitThread(hLibModule, dwExitCode); // contains calls to DLL global destructors, may contain message-loop
...
BOOL C__Handles::FindCloseChangeNotification(HANDLE hChangeHandle)
{
    ...
    BOOL ret = ::FindCloseChangeNotification(hChangeHandle); // may take quite a long time
...
C__Handles::OpenFile(LPCSTR lpFileName, LPOFSTRUCT lpReOpenBuff, UINT uStyle)
{
    ...
    CheckCreate(ret != HFILE_ERROR, __htFile, __hoOpenFile, (HANDLE)(UINT_PTR)ret, GetLastError(), TRUE); // lpFileName is only char even in Unicode version = unusable function (obsolete), I removed it from parameter output
    return ret;
}
                                ...
                                int x, int y, int cx, int cy, UINT uFlags)
{
    ...
    if (ret != hWinPosInfo) // structure reallocation occurred - we must change watched handle value
    {
