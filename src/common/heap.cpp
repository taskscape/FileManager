...
__declspec(allocate(".i_hea$a")) const _PVFV i_heap = (_PVFV)1; // at the beginning of .i_hea section we place variable i_heap

#pragma section(".i_hea$z", read)
__declspec(allocate(".i_hea$z")) const _PVFV i_heap_end = (_PVFV)1; // and at the end of .i_hea section we place variable i_heap_end
...
public:
    ...
    ~C__GCHeapInit()
    {
        ...
        if (_CrtMemDifference(&diff, &start_state, &end_state))
        {
            ...
            // map into memory all modules where memory leaks may be reported,
            // this will display .cpp file names in report (otherwise there would be only "#File Error#")
            for (int i = 0; i < UsedModulesCount; i++)
                hUsedModules[i] = LoadLibraryEx(UsedModules[i], NULL, DONT_RESOLVE_DLL_REFERENCES);

            // output all unreleased blocks
            _CrtMemDumpAllObjectsSince(&start_state);

            // when we have diff, let's output it too
            _CrtMemDumpStatistics(&diff);

            // release mapped modules again
            for (int i = 0; i < UsedModulesCount; i++)
                FreeLibrary(hUsedModules[i]);

            // throw warning messagebox
