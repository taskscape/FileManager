    ...
    if (SHGetFolderPath(NULL, CSIDL_LOCAL_APPDATA, NULL, 0 /* SHGFP_TYPE_CURRENT */, sDbPath) == S_OK)
    {
        ...
        if (pathOK)
        {
            ...
            if (sqlite3_Dyn->OK)
            {
                sqlite3* pDb;
                sqlite3_stmt* pStmt;
                char utf8Select[] = "SELECT data_value FROM data WHERE entry_key = 'local_sync_root_path';"; // UTF8 string (if any extra character is added, it needs to be converted ANSI->UTF8)

                // sqlite3_open_v2 requires UTF8 path, so we convert it from ANSI to UTF8
            ...
            if (sqlite3_Dyn_InOut != NULL)
                *sqlite3_Dyn_InOut = sqlite3_Dyn; // return loaded sqlite.dll for further use (it could have been loaded before calling this function)
            else
...
/*
// returns module (DLL) handle in which the given function address is located
// (gets DLL module handle for specified function address)
// if we want this for currently executing code, this is also a solution (MS specific):
...
void InitShellIconOverlaysAuxAux(CLSID* clsid, const char* name)
{
    ...
    if (CoCreateInstance(*clsid, NULL,
        ...
        iconOverlayIdentifier != NULL) // probably unnecessary test, just for synchronization
    {
        ...
        if (iconOverlayIdentifier->GetOverlayInfo(iconFile, MAX_PATH, &iconIndex, &flags) == S_OK)
        {
            ...
            {
                int priority; // priority: we will query handlers for overlay with the lowest priority number first
                ...
                if (iconOverlay[ICONSIZE_16] != NULL && iconOverlay[ICONSIZE_32] != NULL && iconOverlay[ICONSIZE_48] != NULL)
                {
                    ...
                    {
                        isGoogleDrive = TRUE;
                        // call Google Drive handlers only for subdirectories of the path where Google Drive resides
...
void InitShellIconOverlays()
{
                                         ...
                                         0, KEY_ENUMERATE_SUB_KEYS, &key))) == ERROR_SUCCESS)
    {
        ...
        DWORD i = 0;
        while (1)
        { // sequential enumeration of all icon-overlay-handlers
            FILETIME dummy;
        ...
        // iterate through sorted list of icon-overlay-handlers (Explorer defines handler priority alphabetically)
        // we take only the first 15, Explorer even only the first 11
        for (int s = 0; s < keyNames.Count; s++)
        { // opening icon-overlay-handler key
            HKEY handler;
                        ...
                        if (CLSIDFromString(oleTxtClsId, &clsid) == NOERROR)
                        {
                            ...
                            if (clsIDKey != NULL)
                            {
                                ...
                                if ((errRet = HANDLES_Q(RegOpenKeyEx(clsIDKey, txtClsId, 0, KEY_QUERY_VALUE, &classKey))) == ERROR_SUCCESS)
                                {
                                    ...
                                    {
                                        if (errRet != ERROR_FILE_NOT_FOUND) // this error is reported when handler has no description (which is obviously not an error, because on Vista it applies e.g. to Offline Files)
                                        {
                                ...
                                {
                                    // Petr: after Google Drive update 30.8.2015, the key for GDriveSharedOverlay was missing
                                    //       under CLSID in registry, I found no difference in overlay display vs. Explorer,
                                    //       so I bypassed this annoying message by deleting the GDriveSharedOverlay key
                                    //       from the ShellIconOverlayIdentifiers list
...
BOOL IsNameInListOfDisabledCustomIconOverlays(const char* name)
{
    ...
    {
        static char buf[MAX_PATH]; // also called from Bug Report - we don't want to burden the stack
        ...
        while (*s != 0)
        {
            ...
            if (d >= end)
            {
                d = end - 1; // for longer names (shouldn't happen), we'll overwrite the last character (null-terminator)
...
BOOL AddToListOfDisabledCustomIconOverlays(const char* name)
{
    ...
    static char n[2 * MAX_PATH]; // also called from Bug Report - we don't want to burden the stack
    char* d = n;
    const char* s = name;
    while (*s != 0)
    {
...
CShellIconOverlayItem::~CShellIconOverlayItem()
{
    // VC2015 didn't like __try / __except block in destructor, linker complained in x64 version:
    // error LNK2001: unresolved external symbol __C_specific_handler_noexcept
    // moving to a function solved the problem
    Cleanup();
}
...
BOOL CShellIconOverlays::Add(CShellIconOverlayItem* item /*, int priority*/)
{
    ...
    // sorting by priority is nonsense, MS writes that it uses it only if other prioritization methods
    // fail, in reality it goes alphabetically in the list of overlay handlers; priority is probably used only for
...
void CreateIconReadersIconOverlayIdsAuxAux(CLSID* clsid, const char* name, IShellIconOverlayIdentifier** ids, int i)
{
    ...
    if (CoCreateInstance(*clsid, NULL, CLSCTX_INPROC_SERVER, IID_IShellIconOverlayIdentifier,
        ...
        iconOverlayIdentifier != NULL) // probably unnecessary test, just for synchronization
    {
        // just for the sake of form, call the usual methods (as if we were Explorer and wanted to show those overlays)
        OLECHAR iconFile[MAX_PATH];
        int iconIndex;
        DWORD flags;
                                        ...
                                        BOOL isGoogleDrivePath)
{
    ...
    wPath[MAX_PATH - 1] = 0; // just for safety
    strcpy(aName, name);

    //  SHFILEINFO fi;
    //  if (SHGetFileInfoAux(aPath, 0, &fi, sizeof(fi), SHGFI_ATTRIBUTES))
    //  {
    // GoogleDrive crashes when IsMemberOf() is called concurrently from both icon-readers, one thread allocates,
    // the other deallocates and the heap is corrupted (hard to say why, their bug), the critical section
    // slows down reading in both panels quite a bit (2x), so we try to use it only when GD
    ...
    for (int i = 0; i < Overlays.Count; i++)
    {
        ...
        if (overlay->GoogleDriveOverlay)
        {
            if (!isGoogleDrivePath)
                continue; // call Google Drive handlers only for their directory and its subdirectories (they are slow and crash without added synchronization)
            if (!isGD_CS_entered)
            {
...
void CShellIconOverlays::InitGoogleDrivePath(CSQLite3DynLoadBase** sqlite3_Dyn_InOut, BOOL debugTestOverlays)
{
    ...
    { // test only if we will advertise Google Drive on toolbar and in change drive menu
        ...
        if (!found)
        {
            // Google Drive installs in the version according to Windows (x86 / x64). So Salamander x86
            // on x64 Windows (and vice versa) won't find GD icon-handlers and it's not an error.
#ifdef _WIN64
            if (Windows64Bit)
#else  // _WIN64
            if (!Windows64Bit)
